...
class CodeParser
  ...
  def advance_scanner(scan_position)
    if @scanner.getch.nil?
      handle_unexpected_eof
    else
      @scanner.pos = [@scanner.pos, scan_position + 1].max
    end
  rescue => e
    raise ParseError, "Error advancing scanner: #{e.message}"
  end
  ...
  def parse_relevant_line                                                                                                                                                                         
    ...
    if parse_conditional || parse_define(@scanner) || parse_platform_struct(@scanner)                                                                                                                     
      true                                                                                                                                                                                        
    else                                                                                                                                                                                          
